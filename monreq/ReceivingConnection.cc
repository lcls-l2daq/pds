...
char* ReceivingConnection::receive() {
  ...
  Pds::Dgram datagram;
  int nb = recv(_socket, &datagram, sizeof(datagram), MSG_WAITALL) ;
		
  if (nb==0) {
    printf("ReceivingConnection::receive() socket closed\n");
    return 0;
  }
  if  (nb == -1) {
    perror("recv");
  ...
  if (nb!=sizeof(datagram)) {
    printf("Incomplete datagram header\n");
    abort();
  }
