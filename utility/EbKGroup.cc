...
EbBase::IsComplete EbKGroup::_is_complete( EbEventBase* event, const EbBitMask& serverId)
{
  ...
  //{
  //  printf("EbKGroup::_is_complete(): Sequence 0x%x\n", dg.seq.stamp().fiducials());
  //  if (event->isClientGroupSet())
  //    printf("EbKGroup::_is_complete(): Client Group has been set\n");
  //}
  ...
  return EbK::_is_complete(event, serverId);
}

void EbKGroup::_insert( EbEventBase* event )
{
