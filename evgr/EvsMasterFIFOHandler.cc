...
EvsMasterFIFOHandler::EvsMasterFIFOHandler(Evr&       er, 
  ...
  _src                (src),
  _pool               (sizeof(CDatagram) + sizeof(Xtc) + EvrDataUtil::size( giMaxNumFifoEvent ),16),
  _evtCounter         (0), 
  _evtStop            (0), 
...
{      
  ...
  if ( bEnabled == false ) 
    {
      printf("EvsMasterFIFOHandler::xmit(): [%d] during Disabled, vector %d code %d fiducial 0x%x prev 0x%x last 0x%x timeLow 0x%x.  Dropping\n", 
  ...
  if (codeState.iReportWidth > 0) {
    if (codeState.iReportDelayQ <= 0) {
      ...
      printf("EvsMasterFIFOHandler::xmit(): Can't queue eventcode %d\n",fe.EventCode);
    }
  }
  else {
...
InDatagram* EvsMasterFIFOHandler::l1accept(InDatagram* in)
{
  InDatagram* out = in;
  out->datagram().xtc.extent = sizeof(Xtc);

  do {
    if ( _pool.numberOfFreeObjects() <= 0 )
      {
    ...
    out = new ( &_pool ) CDatagram( in->datagram() ); 
...
Transition* EvsMasterFIFOHandler::enable      (Transition* tr)
{
...
void        EvsMasterFIFOHandler::set_config  (const EvsConfigType* pEvrConfig)
{
  ...
  for ( unsigned int uEventIndex = 0; uEventIndex < pEvrConfig->neventcodes(); uEventIndex++ )
    {
      const EvsCodeType& eventCode = pEvrConfig->eventcodes()[uEventIndex];
      if ( eventCode.code() >= guNumTypeEventCode )
        {
          printf( "EvsMasterFIFOHandler::setEvrConfig(): event code out of range: %d\n", eventCode.code() );
          continue;
        }
      
      EventCodeState& codeState = _lEventCodeState[eventCode.code()];
      codeState.uMaskReadout   |= (1<<eventCode.readoutGroup());
      codeState.bCommand        = false;
      codeState.iDefReportDelay = 0;
...
Transition* EvsMasterFIFOHandler::config      (Transition* tr)
{
  reset();
...
Transition* EvsMasterFIFOHandler::endcalib    (Transition* tr)
{
  ++uNumBeginCalibCycle;
  return tr;
}
...
void EvsMasterFIFOHandler::startL1Accept(const FIFOEvent& fe, bool bEvrDataIncomplete)
{    
  ...
  if ( pthread_equal(tid, pthread_self()) )
    {
      printf("EvsMasterFIFOHandler::startL1Accept(): [%d] Re-entry call for vector %d fiducial 0x%x prev 0x%x Incomplete %c last 0x%x timeLow 0x%x code %d\n",
  ...
  if ( pthread_mutex_lock(&mutex) )
    {
      printf("EvsMasterFIFOHandler::startL1Accept(): pthread_mutex_lock() failed\n");
      return;
    }
  ...
  if ( !(_uMaskReadout != 0 || _ncommands) )
    {
      if ( pthread_mutex_unlock(&mutex) )
        printf( "EvsMasterFIFOHandler::startL1Accept(): pthread_mutex_unlock() failed\n" );
            
      printf("EvsMasterFIFOHandler::startL1Accept(): No readout/commands detected. Possibly called by two threads.\n");
      tid = -1;
      return;          
    }
  ...
  if (fe.TimestampHigh == 0 && uFiducialPrev < 0x1fe00) // Illegal fiducial wrap-around
    {
      printf("EvsMasterFIFOHandler::startL1Accept(): [%d] vector %d fiducial 0x%x prev 0x%x Incomplete %c last 0x%x timeLow 0x%x code %d\n",
             uNumBeginCalibCycle, _evtCounter, fe.TimestampHigh, uFiducialPrev, (bEvrDataIncomplete?'Y':'n'),
             _lastFiducial, fe.TimestampLow, fe.EventCode );
    }
  ...
  if (_uMaskReadout == 0) 
  {
  ...
  else 
    {
      ...
      if ( ! _evrL1Data.isDataWriteReady() )
        {
          printf( "EvsMasterFIFOHandler::startL1Accept(): Previous Evr Data has not been transferred out.\n"
...
void EvsMasterFIFOHandler::clear()        
{ 
...
void EvsMasterFIFOHandler::nextEnable()
{ 
  _evrL1Data.reset();
...
void EvsMasterFIFOHandler::reset()        
{
  clear();    
  _evrL1Data.reset();
  _evtCounter = 0;
...
int EvsMasterFIFOHandler::getL1Data(const ClockTime& iTriggerCounter, const EvrDataUtil* & pEvrData, bool& bOutOfOrder) 
{  
  ...
  if ( ! _evrL1Data.isDataReadReady() )
    {
      printf( "EvsMasterFIFOHandler::getL1Data(): No L1 Data ready: ");
  ...
  if ( _evrL1Data.getCounterRead() > iTriggerCounter ) //  test if data is later than trigger
    {
      printf( "EvsMasterFIFOHandler::getL1Data(): Missing L1 Data: ");
      _print_L1(iTriggerCounter,_evrL1Data);
      return 2;      
    }
  ...
  if ( iDataIndex != -1 )
    {
       ...
       *   used later
       */      
      printf( "EvsMasterFIFOHandler::getL1Data(): Recovered Out-of-order L1 Data: Data Index = %d, ",iDataIndex);
      _print_L1(iTriggerCounter, _evrL1Data);
  ...
  printf( "EvsMasterFIFOHandler::getL1Data(): Missing Triggers: ");
  _print_L1(iTriggerCounter, _evrL1Data);
  ...
  printf("EvsMasterFIFOHandler::getL1Data(): Dropped %d Data: ", iDataDropped);
  _print_L1(iTriggerCounter, _evrL1Data);
  return 4; // Will set Dropped Contribution damage for L1 Data
}
  
void EvsMasterFIFOHandler::releaseL1Data() 
{ 
  /*
  ...
  if (!bThreadSyncInit)
    {
      ...
      if ( pthread_sigmask(SIG_BLOCK, &sigetHoldIO, NULL) < 0 )
        perror( "EvsMasterFIFOHandler::releaseL1Data(): pthread_sigmask() failed" );
...
int EvsMasterFIFOHandler::addFifoEventCheck( EvrDataUtil& evrData, const Pds::EvrData::FIFOEvent& fe )
{
...
int EvsMasterFIFOHandler::updateFifoEventCheck( EvrDataUtil& evrData, const Pds::EvrData::FIFOEvent& fe )
{
...
void EvsMasterFIFOHandler::addCommand( const FIFOEvent& fe )
{
