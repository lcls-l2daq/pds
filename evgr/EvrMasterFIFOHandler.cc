...
void EvrMasterFIFOHandler::fifo_event(const FIFOEvent& fe)
{
  ...
  */
  ...
  //!!!debug
  //printf("event %d fiducial 0x%x offset %d readout [0x%x]\n", uEventCode, fe.TimestampHigh, fe.TimestampLow, codeState.uMaskReadout);
...
InDatagram* EvrMasterFIFOHandler::l1accept(InDatagram* in)
{
  ...
  do {
    if ( _poolEvrData.numberOfFreeObjects() <= 0 )
      {
        printf( "EvrMasterFIFOHandler::l1accept(): Pool is full, so cannot provide buffer for new datagram\n" );
        break;
      }
    ...
    if ( bDataInc )
      {
        printf( "EvrMasterFIFOHandler::l1accept(): [%d] Incomplete data with vector %d fiducial 0x%x prev 0x%x last 0x%x\n",
        ...
        if ( evrHasEvent(_er) )
          {
            printf( "EvrMasterFIFOHandler::l1accept(): Found unprocessed FIFO event, which may cause missing triggers\n" );
        ...
        if ( (uFiducialCur <= uFiducialPrev && uFiducialPrev < uFiducialCur+iFiducialWrapAroundDiffMin) )
          {
            ...
            printf( "EvrMasterFIFOHandler::l1accept(): seq 0x%x followed 0x%x\n", uFiducialCur, uFiducialPrev );
            //out->datagram().xtc.damage.increase(Pds::Damage::UserDefined);
    ...
    if ( bDataFull )
      {
        printf( "EvrMasterFIFOHandler::l1accept(): Too many FIFO events recieved, so L1 data buffer is full and incomplete.\n" );
    ...
    {
      printf( "EvrMasterFIFOHandler::l1accept(): [%d] vector %d fiducial 0x%x prev 0x%x\n",
              uNumBeginCalibCycle, iVector, uFiducialCur, uFiducialPrev );
      printf( "EvrMasterFIFOHandler::l1accept() data dump start (size = %u bytes)\n", evrData.size() );
      evrData.printFifoEvents();
      printf( "EvrMasterFIFOHandler::l1accept() data dump end\n\n" );
    }
    //
