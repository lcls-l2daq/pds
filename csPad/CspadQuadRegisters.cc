...
unsigned CspadQuadRegisters::read(unsigned index, CspadDirectRegisterReader* rdr) {
  Pds::Pgp::RegisterSlaveExportFrame::FEdest dest = (Pds::Pgp::RegisterSlaveExportFrame::FEdest)index;
  uint32_t* up = (uint32_t*) this;
  unsigned i=0;
  int ret = 0;
  do {
    ret = rdr->readRegister(dest, _regLoc[i].addr, 0xf400+(index<<4)+i, up+i);
