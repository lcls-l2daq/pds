...
unsigned CspadConcentratorRegisters::read(CspadDirectRegisterReader* rdr) {
  Pds::Pgp::RegisterSlaveExportFrame::FEdest dest = Pds::Pgp::RegisterSlaveExportFrame::CR;
  uint32_t* up = (uint32_t*) this;
  unsigned i=0;
  int ret = 0;
  do {
    ret = rdr->readRegister(dest, _regLoc[i].addr, 0xfc00+i, up+i);
