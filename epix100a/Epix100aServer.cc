...
class Task;
class TaskObject;
class Pds::Epix100a::Epix100aDestination;
class DetInfo;
     ...
     if (_firstFetch) {
       ...
       if (max > 100) {
         if ( (diff >= ((peak<<1)-(peak>>1))) || (diff <= ((peak>>1))+(peak>>2)) ) {
           printf("Epix100aServer::fetch exceptional period %3llu, not %3u, frame %5u, frames since last %5u, ms since last %5u, ms/f %6.3f\n",
