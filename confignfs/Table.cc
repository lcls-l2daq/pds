...
static const std::string _unassigned = std::string("Unassigned");
...
TableEntry::TableEntry() : _name("None"), _key(_unassigned), _changed(false) {}

TableEntry::TableEntry(const string& name) : _name(name), _key(_unassigned), _changed(false) {}
...
bool TableEntry::initial() const { return _key==_unassigned; }
...
void Table::copy_top_entry(const string& dst, const string& src)
{
  for(list<TableEntry>::iterator iter=_entries.begin(); iter!=_entries.end(); iter++)
    if (iter->name()==src) {
      _entries.push_back(TableEntry(dst,_unassigned,iter->entries())); 
      break;
    }
}
...
void Table::set_entry(const string& top, const FileEntry& e)
{
  ...
  if (entry)
    ...
    TableEntry te(top,_unassigned,e);
