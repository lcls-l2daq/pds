...
static Pds_ConfigDb::DeviceEntry DeviceEntry_load(const char*& p)
{
  unsigned _log=0,_phy=0;
  XML_iterate_open(p,tag)
    if      (tag.name == "_log")
      _log = Pds_ConfigDb::XML::IO::extract_i(p);
    else if (tag.name == "_phy")
      _phy = Pds_ConfigDb::XML::IO::extract_i(p);
  XML_iterate_close(DeviceEntry,tag);

  uint64_t src = _log;
  src <<= 32;
  src  |= _phy;
  return Pds_ConfigDb::DeviceEntry(src);
}

static void DeviceEntry_save(const Pds_ConfigDb::DeviceEntry& e, char*& p)
{
  XML_insert(p, "unsigned", "_log" , Pds_ConfigDb::XML::IO::insert(p,e.log()));
  XML_insert(p, "unsigned", "_phy" , Pds_ConfigDb::XML::IO::insert(p,e.phy()));
}
...
void Device::load(const char*& p)
{
  ...
  XML_iterate_open(p,tag)
    if      (tag.name == "_name")
      ...
      _src_list.push_back(DeviceEntry_load(p));
    }
...
void Device::save(char*& p) const
{
  ...
  for(list<DeviceEntry>::const_iterator it=_src_list.begin(); it!=_src_list.end(); it++) {
    XML_insert(p, "DeviceEntry", "_src_list", DeviceEntry_save(*it,p) );
  }
}
...
{    
  ...
  entry = _table.get_top_entry(string(PdsDefs::GlobalCfg_name()));
  if (entry) {
    for(list<FileEntry>::const_iterator iter=entry->entries().begin(); iter!=entry->entries().end(); iter++) {
      UTypeName utype(iter->name());
      string tlink = typelink(utype,iter->entry());
...
{    
  ...
  const TableEntry* gentry = _table.get_top_entry(std::string(PdsDefs::GlobalCfg_name()));
  if (gentry)
