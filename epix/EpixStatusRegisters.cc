...
void EpixStatusRegisters::print() {
  printf("EpixStatusRegisters: version(0x%x)\n", version);
  lane.print();
...
int EpixStatusRegisters::read() {
  int ret = 0;
  EpixDestination d;
  d.dest(EpixDestination::VC2);
  ...
  if (ret != 0) {
    printf("\tEpix Status Registers encountered error while reading!\n");
  }
  return ret;
}

unsigned EpixStatusRegisters::readVersion(uint32_t* retp) {
  EpixDestination d;
  d.dest(EpixDestination::VC2);
