...
void FexampStatusRegisters::print() {
  printf("FexampStatusRegisters: version(0x%x)\n", version);
  lane.print();
...
int FexampStatusRegisters::read() {
  int ret = 0;
  FexampDestination d;
  d.dest(FexampDestination::VC2);
  ...
  if (ret != 0) {
    printf("\tFexamp Status Registers encountered error while reading!\n");
  }
  return ret;
}

unsigned FexampStatusRegisters::readVersion(uint32_t* retp) {
  FexampDestination d;
  d.dest(FexampDestination::VC2);
