...
#include "pds/fexamp/FexampExternalRegisters.hh"
...
namespace Pds {
  namespace Fexamp {
    ...
    class ASIC;
     ...
     static uint32_t externalAddrs[FexampConfigType::NumberOfRegisters] = {
        // NB if these are to be included they should be uncommented ConfigV1.cc/hh and FexampConfigurator.cc
        0x00,
        0x01,
        0x02,
        0x03,
...
//        0x07,
    ...
    FexampConfigurator::FexampConfigurator( FexampConfigType* c, int f, unsigned d) :
                   Pds::Pgp::Configurator(f, d),
                   _testModeState(0), _config(c), _rhisto(0) {
       printf("FexampConfigurator constructor _config(%p)\n", _config);
    ...
    bool FexampConfigurator::_flush(unsigned index=0) {
      ...
      bool ret = false;
      printf("\n\t--flush-%u-", index);
      _d.dest(FexampDestination::Internal);
      ...
      failCount = 0;
      FexampExternalRegisters eregs = FexampExternalRegisters::FexampExternalRegisters(_pgp);
      _d.dest(FexampDestination::External);
    ...
    unsigned FexampConfigurator::configure(unsigned mask) {
      ...
      if (_pgp->writeRegister(&_d, ClearFrameCountAddr, ClearFrameCountValue)) {
        printf("FexampConfigurator::configure writing clear frame count failed!\n");
        ret |= 1;
      }
      ret <<= 1;
      if (_pgp->writeRegister(&_d, ClearFrameCountAddr, 0)) {
        printf("FexampConfigurator::configure clearing clear frame count failed!\n");
        ret |= 1;
      }
      ret <<= 1;
      _d.dest(FexampDestination::Internal);
    ...
    unsigned FexampConfigurator::writeConfig() {
      ...
      for (unsigned i=0; i<FexampConfigType::NumberOfRegisters; i++) {
        if (_pgp->writeRegister(&_d, externalAddrs[i], u[i] & FexampConfigType::rangeHigh((FexampConfigType::Registers)i))) {
        ...
        if ((externalAddrs[i] < ReadoutTimingUpdateAddr) && (externalAddrs[i+1] > ReadoutTimingUpdateAddr)) {
          if (_pgp->writeRegister(&_d, ReadoutTimingUpdateAddr, 1)) {
            printf("FexampConfigurator::writeConfig failed writing on ReadoutTimingUpdateAddr\n");
            ret = Failure;
          }
        }
      }
    ...
    unsigned FexampConfigurator::checkWrittenConfig(bool writeBack) {
      ...
      uint32_t* u = (uint32_t*)_config;
      unsigned ret = Success;
      unsigned size = FexampConfigType::NumberOfRegisters;
...
//      for (unsigned i=0; i<7; i++) printf("0x%x (%u)\n", myBuffer[i], myBuffer[i]); printf("0x%x (%u)\n", myBuffer[10], myBuffer[10]);
      for (unsigned i=0; i<size; i++) {
        if (_pgp->readRegister(&_d, externalAddrs[i], 0x101, myBuffer+i)) {
          ...
          ret |= Failure;
        } else if ((myBuffer[i] & (uint32_t)FexampConfigType::rangeHigh((FexampConfigType::Registers)i)) != u[i]) {
          printf("FexampConfigurator::checkWrittenConfig failed, 0x%x!=0x%x at offset %u. %sriting back to config.\n",
              ...
              writeBack ? "W" : "Not w");
          ret |= Failure;
//          if (writeBack) u[i] = myBuffer[i] & FexampConfigType::rangeHigh((FexampConfigType::Registers)i);
        }
      }
      return ret;
    }

    unsigned FexampConfigurator::writeASICs() {
      _d.dest(FexampDestination::External);
      for (unsigned index=0; index<FexampConfigType::NumberOfASICs; index++) {
        if (_pgp->writeRegisterBlock(
            ...
            sizeof(FexampASIC)/sizeof(uint32_t))) {
          printf("FexampConfigurator::writeASICs failed on ASIC %u\n", index);
          return Failure;
        }
      }
      if (_pgp->writeRegister(
          ...
          ASIC_triggerAddr,
          ASIC_triggerValue)) {
        printf("FexampConfigurator::writeASICs failed on writing the trigger!\n");
        return Failure;
      }
      return Success; //checkWrittenASICs(true);
    }

    unsigned FexampConfigurator::checkWrittenASICs(bool writeBack) {
      _d.dest(FexampDestination::External);
      uint32_t* u;
      unsigned size = sizeof(FexampASIC)/sizeof(uint32_t);
      uint32_t myBuffer[size];
      unsigned ret = Success;
      for (unsigned index=0; index<FexampConfigType::NumberOfASICs; index++) {
        u = (uint32_t*)  &(_config->ASICs()[index]);
        if (_pgp->readRegister(&_d, ASIC_topBaseAddr - index * ASIC_addrOffset, 0xa000+index, myBuffer, size)){
          printf("FexampConfigurator::checkWrittenASICs read back failed on ASIC %u\n", index);
          return Failure;
        }
        for (unsigned i=0; i<size; i++) {
          if (myBuffer[i] != u[i]) {
            ret = Failure;
            printf("FexampConfigurator::checkWrittenASIC failed, 0x%x!=0x%x at offset %u. %sriting back to config.\n",
                u[i],
                myBuffer[i],
                i,
                writeBack ? "W" : "Not w");
    ...
    void FexampConfigurator::dumpFrontEnd() {
      ...
      if (_debug & 0x100) {
        _d.dest(FexampDestination::Internal);
        FexampInternalRegisters regs;
        ret = _pgp->readRegister(
            &_d,
            RegisterAddr,
            0x666,
            (uint32_t*)&regs,
            sizeof(FexampInternalRegisters)/sizeof(unsigned));
        if (ret == Success) {
          regs.print();
        } else {
          printf("\tFexamp Internal Registers could not be read!\n");
        }
        FexampExternalRegisters eregs = FexampExternalRegisters::FexampExternalRegisters(_pgp);
      ...
      if (_debug & 0x400) {
        ...
        if (Failure == checkWrittenASICs(false)) {
          printf("FexampConfigurator::checkWrittenASICs() FAILED !!!\n");
        }
      }
      clock_gettime(CLOCK_REALTIME, &end);
      uint64_t diff = timeDiff(&end, &start) + 50000LL;
