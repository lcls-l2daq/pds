...
void Epix10kStatusRegisters::print() {
  printf("Epix10kStatusRegisters: version(0x%x)\n", version);
  lane.print();
...
int Epix10kStatusRegisters::read() {
  int ret = 0;
  Epix10kDestination d;
  d.dest(Epix10kDestination::VC2);
  ...
  if (ret != 0) {
    printf("\tEpix10k Status Registers encountered error while reading!\n");
  }
  return ret;
}

unsigned Epix10kStatusRegisters::readVersion(uint32_t* retp) {
  Epix10kDestination d;
  d.dest(Epix10kDestination::VC2);
